...
namespace App\Livewire\MedicalRep\SampleList;
...
class SampleListComponent extends Component
{
    use WithFileUploads;
    ...
    public function openAddToCartModal($productId)
    {
        ...
        if (!$role?->hasPermission('add_to_cart') && $user->role_id > 2) {
            $this->dispatch('show-notification', 'You don\'t have permission to Add Products to cart!', 'error');
            return;
        }
        ...
        $this->units = ProductUnit::with('unit')
            ...
            ->map(function ($productUnit) {
                return [
                    ...
                    'unit_name' => $productUnit->unit->unit_name ?? 'Unknown', // safe null check
        ...
        // Initialize quantities and units for each location
        $baseUnit = collect($this->units)->firstWhere('is_base_unit', true);

        foreach ($this->locations as $location) {
            $locationId = $location['id'];

            // Check if product already exists in cart for this location
            $existingCartItem = Cart::where('product_id', $productId)
                ->where('location_id', $locationId)
                ->first();

            if ($existingCartItem) {
                $this->locationQuantities[$locationId] = $existingCartItem->quantity;
                $this->locationUnits[$locationId] = $existingCartItem->unit_id;
            } else {
                // Get PAR quantity from mycatalog if exists
    ...
    public function addToCartBulk()
    {
        ...
        foreach ($this->locations as $location) {
            $locationId = $location['id'];
            $quantity = $this->locationQuantities[$locationId] ?? 0;
            $unitId = $this->locationUnits[$locationId] ?? null;

            // Skip if quantity is 0 or unit is not selected
            if ($quantity <= 0 || !$unitId) {
                continue;
            }

            // Check if product already exists in cart
            $existingCartItem = Cart::where('product_id', $this->selectedProductId)
                ->where('location_id', $locationId)
                ->first();

            // Calculate price based on unit conversion
            $selectedUnit = collect($this->units)->firstWhere('unit_id', $unitId);
            $basePrice = $this->selectedProductCost;

            if ($selectedUnit && !$selectedUnit['is_base_unit']) {
                if ($selectedUnit['operator'] === '*') {
                    $basePrice = $basePrice * $selectedUnit['conversion_factor'];
                } elseif ($selectedUnit['operator'] === '/') {
                    $basePrice = $basePrice / $selectedUnit['conversion_factor'];
                }
            }
            ...
            if ($existingCartItem) {
                ...
                $auditService->logCartChanges(
                    $existingCartItem->id,
                    'Updated',
                    'Cart item quantity/unit updated'
                );
                $updatedCount++;
            } else {
        ...
        $this->dispatch('close-modal', 'add-product-to-cart-locations');

        $message = '';
        if ($addedCount > 0)
            $message .= "$addedCount location(s) added. ";
        if ($updatedCount > 0)
            $message .= "$updatedCount location(s) updated.";

        $this->dispatch('show-notification', $message ?: 'No changes made.', $addedCount > 0 || $updatedCount > 0 ? 'success' : 'info');
    }
    ...
    public function render()
    {
        $organization_suppliers = Supplier::orderBy('supplier_name')->where('is_active', true)
            // ->where('verified', true)
            ->get();
        return view('livewire.medical-rep.sample-list.sample-list-component', [
            'organization_suppliers' => $organization_suppliers,
        ]);
    }
}
