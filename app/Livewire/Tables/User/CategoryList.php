...
final class CategoryList extends PowerGridComponent
{
    ...
    {
        return Category::query()->where('organization_id', auth()->user()->organization_id)->where('is_active', true)->with(['organization', 'subcategories']);
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('category_name')
            ->add('subcategories', function ($model) {
                $count = $model->subcategories->count();
                $names = $model->subcategories->pluck('subcategory')->implode(', ');
                return '<span title="' . $names . '">' . $count . '</span>';
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Sub Categories', 'subcategories')
