...
final class OrganizationsList extends PowerGridComponent
{
    public string $tableName = 'organizations-list-kvvozg-table';

    public bool $showFilters = true;
    public int $isRepOrg = 0;
    ...
    public function datasource(): Builder
    {
        $query = Organization::query()
            ...
            ->where('organizations.is_rep_org', $this->isRepOrg)
            ->with('plan')
            ->orderBy('name', 'asc');
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('organization_code', function ($row) {
                return '
                <span 
                    class="px-2.5 py-0.5 "
                >
                    ' . e($row->organization_code) . '
                </span>
            ';
            })
            ->add('organization_code_export', fn($row) => $row->organization_code)
            ...
            ->add('name_export', fn($row) => $row->name)
            ->add('plan.name') // Directly return the plan_name
            ->add('email')
            ->add('users_count', function ($model) {
                return $model->users()->where('system_locked', false)->where('is_active', true)->count();
            })
            ->add('locations_count', function ($model) {
                return $model->locations()->where('is_active', true)->count();
            })
            ->add('phone')
    ...
    ';
            ...
            ->add('open_tickets_export', fn($model) => $model->openTickets()->count())
    ...
    {
        $practiceLabel = $this->isRepOrg ? 'MedRep Practice' : 'Practice';
    $emailLabel = $this->isRepOrg ? 'MedRep Email' : 'Practice Email';
        ...
        return [
            ...
            //     ->searchable()
            //     ->hidden(),
            Column::make('ID', 'organization_code')
                ->visibleInExport(false) // Display organization_code
                ->headerAttribute('max-w-[80px]', 'padding:0 !important; margin: 0 !important; min-width: 0; white-space: normal !important; width: 80px;')
                ->bodyAttribute('max-w-[80px]', 'padding:0 !important; margin: 0 !important; min-width: 0; white-space: normal !important; width: 80px;')

                ->searchable(),
            Column::make('ID', 'organization_code_export')
                ->visibleInExport(true)
                ->hidden(),
            Column::make($practiceLabel, 'name')
            ...
            Column::make($practiceLabel, 'name_export')
                ->visibleInExport(true)
                ->hidden(),

            Column::make($emailLabel, 'email')
            ...
            Column::make('Open Tickets', 'open_tickets_export')
                ->visibleInExport(true)
                ->hidden(),
