...
class InventoryAdjustReportComponent extends Component
{
    ...
    public function mount()
    {
        if (auth()->user()->role_id == 1) {
            ...
            $this->organizations = Organization::where('is_active', true)->where('is_deleted', 0)
                ->where('is_rep_org', 0)->orderBy('name')->get();
        } else {
            // Non-admin: load locations for their organization
    ...
    {
        $this->dispatch('inventoryAdjustOrganizationFilterChanged', $id);
    }

    public function updatedSelectedLocation($id)
    {
        $this->dispatch('inventoryAdjustLocationFilterChanged', $id);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.reports.inventory-adjust-report-component', [
