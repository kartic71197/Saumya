<div>
    <div class="flex justify-between items-center gap-2 pt-3 border-b-2 border-gray-900/10 pb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('View and create the samples you need.') }}
            </p>
        </div>
        <div class="flex justify-between items-center gap-3">
            ...
            <x-primary-button class="min-w-36 flex justify-center items-center" x-data="{ loading: false }"
                ...
                x-bind:disabled="loading">
                <!-- Button Text -->
                <span x-show="!loading">{{ __('+ Add Sample') }}</span>
                <!-- Loader (Spinner) -->
                <span x-show="loading" class="flex justify-center items-center w-full">
    ...
    <div class="text-xs">
        <livewire:tables.medicalrep.sample-list-table />
    </div>
    @include('livewire.medical-rep.sample-list.modals.add-product-modal')
    @include('livewire.medical-rep.sample-list.modals.product-form-modal')
    @include('livewire.medical-rep.sample-list.modals.import-products-modal')
    @include('livewire.medical-rep.sample-list.modals.edit-product-modal')
    @include('livewire.medical-rep.sample-list.modals.cart-product-modal')

    <!-- Notifications Container -->
    <div class="fixed top-24 right-4 z-50 space-y-2">
        @foreach($notifications as $notification)
            <div wire:key="{{ $notification['id'] }}" x-data="{ show: true }" x-init="setTimeout(() => {show = false;$wire.removeNotification('{{ $notification['id'] }}');
            }, 3000)" x-show="show"
