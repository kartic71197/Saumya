<div class="w-full p-3 bg-white rounded-lg text-sm">
    ...
    <div class="flex items-center justify-between border-b border-gray-100 pb-2 mb-3">
        ...
        <!-- commenting out time filter for now -->
        <!-- <div class="flex items-center gap-2">
    ...
    <div class="top-pickups-list space-y-0.5 mt-2">
        @foreach ($top_picks ?? [] as $index => $pickup)
            <div
                ...
                <div class="col-span-2 product-clickable" data-product-id="{{ $pickup->product->id }}">
                    <div class="flex items-center gap-2">
                ...
                <div class="flex flex-1 items-center">
                    <span class="text-xs text-gray-700 truncate"
                        title="{{ $pickup->product->supplier->supplier_name ?? 'N/A' }}">
...
$(document).ready(function() {
    ...
    // -----------------------
    let selectedLocation = $('#locDropdownBtn').data('id') || 0;
    ...
    function updateTopPickupsList(pickups) {
        ...
        pickups.forEach((pickup, index) => {
            ...
            const productName = pickup.product?.product_name || 'Unknown Product';
            const productCode = pickup.product?.product_code || 'N/A';
            const supplierName = pickup.product?.supplier?.supplier_name || 'N/A';
            const locationName = pickup.location_name || 'N/A';
                ...
                <div class="grid grid-cols-6 gap-2 p-2 border border-transparent hover:border-green-100 hover:bg-green-50/30 rounded-md transition cursor-pointer">
                    ...
                    <div class="flex items-center">
                        <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 rounded text-xs truncate">
                            ...
                            <span class="truncate">${locationName.length > 8 ? locationName.slice(0, 8) + '...' : locationName}</span>
                        </span>
                    </div>
    ...
    function refreshTopPickups() {
        ...
        fetch(`/top-pickups/filter?${params.toString()}`, {
            headers: {
    ...
    $('.loc-option').on('click', function() {
        ...
        const locName = $(this).data('name') || 'All Locations';
        $('#locDropdownBtn div').text(locName);
        refreshTopPickups();
    });
    ...
    $(document).on('click', '.reset-filters', function() {
        ...
        $('#locDropdownBtn div').text('All Locations');
        $('#fromDate').val('');
        $('#toDate').val('');
