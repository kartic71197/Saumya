...
<script>
    ...
    //removed organization selection as not needed
    let selectedLocId = '0';
    ...
    $(document).ready(function () {
        ...
        $('.loc-option').on('click', function () {
            selectedLocId = $(this).data('id');
        });

        console.log('selectedLocId at supplier ', selectedLocId)
    });
    ...
    function loadSupplierChart(timeFilter = '') {
        ...
        console.log('Supplier API params', params);

        $.get('/supplier-costs', params, function (data) {
            console.log('Supplier data', data);
            processSupplierData(data);
        }).fail(function (xhr) {
            console.error('Supplier API failed', xhr.responseText);
            showEmptyState();
        });
    }
...
{{-- <script>
    ...
    function initializeEventListeners() {
        ...
        $('#sort-option-time').change(function() {
            const timeFilter = $('#sort-option-time').val();
            console.log("[DEBUG] Time filter selected:", timeFilter);
            loadSupplierChart(timeFilter);
        });
    ...
    function loadSupplierChart(timeFilter = '') {
        ...
        const url = timeFilter ?
            `/supplier-costs?time=${encodeURIComponent(timeFilter)}` :
            '/supplier-costs';
    ...
    function renderChart() {
        console.log("Rendering chart with data:", supplierData);
        $('#chart-container').removeClass('hidden');
        ...
        const processedData = filteredSupplierData.map(item => ({
            name: item.name || 'Unknown',
            cost: Number(item.cost) || 0  // Force number conversion and default to 0
        }));
    ...
    function updateSupplierStats(supplierList) {
        console.log('Called updateSupplierStats with:', supplierList);

        processSupplierData(supplierList);
        if ($('#chart-container').is(':visible') && filteredSupplierData.length > 0) {
            renderChart();
        }
    }
</script> --}}
