...
<div class="px-2 py-1 rounded flex items-center space-x-1 justify-end bg-gray-50"
    ...
    <span id="recentOrdersOrganizationDisplay" class="text-xs font-medium text-gray-500">All Practices</span>
</div>
...
<script>
function updateRecentPurchaseOrders(orders, organizationName) {
    const organizationEl = document.getElementById('recentOrdersOrganizationDisplay');
    if (organizationEl) organizationEl.textContent = organizationName ?? 'All Practices';
    ...
    orders.forEach(order => {
        ...
        const organization = order.organization?.name ??'Unknown';
        ...
        container.innerHTML += `
            ...
            <div class="hidden lg:grid gap-3 items-center px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200 rounded-lg border-b border-gray-100 dark:border-gray-700" style="grid-template-columns: 140px 100px 90px 90px 1fr;">
                ...
                <div class="text-xs text-gray-700 dark:text-gray-300 truncate">${organization}</div>
            ...
            <div class="lg:hidden bg-white dark:bg-gray-800 rounded-lg p-3 mb-2 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow duration-200">
                ...
                <div class="flex justify-between items-center mb-3 text-[10px]">
                    <div class="flex items-center space-x-1">
                        ...
                        <span class="text-gray-700 dark:text-gray-300">${organization}</span>
                    </div>
                    <div class="flex items-center space-x-1">
                        <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    updateRecentPurchaseOrders(@json($recent_purchase_orders_list), 'All Practices');
});
</script>
