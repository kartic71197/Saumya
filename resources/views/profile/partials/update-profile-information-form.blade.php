<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <div class="overflow-x-auto flex flex-wrap py-2">
                @foreach ($avatars as $avatar)
                    <label class="flex-shrink-0">
                        <input type="radio" name="avatar" value="{{ $avatar }}"
                            @if ($user->avatar === $avatar) checked @endif class="hidden peer">
                        <img src="{{ asset('avatars/' . $avatar) }}"
                            class="w-24 h-24 rounded-full border-2 border-transparent peer-checked:border-blue-500 cursor-pointer"
                            alt="Avatar">
                    </label>
                @endforeach
            </div>
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    class="text-sm text-primary dark:text-gray-400">{{ __('Saved.') }}</p>
            @endif
        </div>
    </form>
</section>
